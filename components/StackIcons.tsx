...
export default function StackIcons() {
  return (
    ...
    >
      <p className="text-sm uppercase tracking-widest text-muted mb-6 font-mono text-center">
        Tools I work with
      </p>
      <motion.div
      ...
      >
        {stack.map((tool) => (
          <motion.span
            key={tool}
            variants={fadeUpItem}
            className="px-4 py-2 border border-edge rounded-full text-sm text-muted font-mono hover:border-accent/50 transition"
          >
            {tool}
          </motion.span>
        ))}
